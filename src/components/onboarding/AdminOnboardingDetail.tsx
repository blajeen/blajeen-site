...
export function AdminOnboardingDetail({ id }: { id: string }) {
  ...
  const emailEvent = bundle?.events.find((entry) => entry.type.startsWith('EMAIL_'));
    ...
    <div className="mx-auto w-full max-w-[110rem] px-[var(--gutter)] py-12">
      ...
      <div className="mt-8 grid gap-8 xl:grid-cols-[minmax(0,1fr)_22rem]">
        ...
        <aside className="space-y-6 xl:sticky xl:top-24 xl:self-start">
          {emailEvent ? <section className={`rounded-[var(--radius-panel)] border p-5 ${emailEvent.type === 'EMAIL_SENT' ? 'border-signal/40 bg-signal/5' : 'border-amber-300/30 bg-amber-300/5'}`}><p className="tecnica text-signal">RESUMO DO BRIEFING</p><p className="mt-3 text-sm leading-relaxed text-mineral">{emailEvent.type === 'EMAIL_SENT' ? 'O resumo completo foi enviado para o e-mail da Blajeen Labs.' : emailEvent.type === 'EMAIL_PENDING' ? 'O resumo está salvo. O envio automático aguarda a configuração do serviço de e-mail.' : 'O envio por e-mail falhou, mas o resumo continua salvo e pode ser baixado acima.'}</p></section> : null}
